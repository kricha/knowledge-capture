...
## Context
The first implementation idea was to mark timed-out invoices as `pending_review`, but tests showed that path would require manual cleanup for ordinary provider timeouts.
...
- Added tests for timeout retry and hard-decline failure.
- Ran `npm test -- invoice`.
...
- For this repo, invoice provider timeouts are retryable; hard declines are terminal failures.
```
